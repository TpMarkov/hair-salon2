...
const Navbar = () => {
  const { adminToken, setAdminToken, sideBarCollapsed, setSideBarCollapsed } = useContext(AdminContext)
  ...
  const navItems = [
    ...
    { path: '/services-list', icon: Briefcase, label: 'Services' },
  ]
  ...
  return (
    <>
      ...
      <nav className={`admin-nav ${sideBarCollapsed ? 'collapsed' : ''} ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="admin-nav-inner flex flex-col h-full">
          {/* Logo Section */}
          <div className="admin-logo-container relative mb-4">
            <img className="w-12 md:w-24 cursor-pointer" src={"/images/logo.png"} alt="admin-logo" onClick={() => navigate('/')} />
            {!sideBarCollapsed && <span className="admin-badge">Admin Panel</span>}
            ...
            >
              <ChevronLeft className={`w-4 h-4 text-gray-400 transition-transform duration-300 ${sideBarCollapsed ? 'rotate-180' : ''}`} />
            </button>
          </div>

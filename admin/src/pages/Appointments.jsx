                ...
                <div className="flex-1 bg-white border border-gray-200 rounded-xl shadow-sm text-sm max-h-[80vh] min-h-[60vh] overflow-y-auto">
                    ...
                    {filteredAppointments && filteredAppointments.length > 0 ? (
                        filteredAppointments.map((item, index) => (
                            <div className="flex flex-col sm:grid grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] gap-4 items-start sm:items-center text-gray-500 py-4 px-4 sm:px-6 border-b border-gray-100 hover:bg-gray-50 transition-all" key={index}>
                                ...
                                <div className="flex items-center gap-3 w-full sm:w-auto">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="sm:hidden text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">#{index + 1}</span>
                                        </div>
                                ...
                                <div className="flex items-center justify-between w-full sm:hidden mt-1">
                                    ...
                                    <div className="flex flex-col items-end">
                                        ...
                                        {item.cancelled
                                            ? <span className='px-3 py-1 rounded-full text-red-600 bg-red-50 text-xs font-semibold border border-red-100'>Cancelled</span>
                                            : <span className='px-3 py-1 rounded-full text-green-600 bg-green-50 text-xs font-semibold border border-green-100'>Confirmed</span>
                                        }
                                    </div>
                                </div>
                                ...
                                <div className="flex max-sm:hidden">
                                    {item.cancelled
                                        ? <span className='px-2.5 py-1 rounded-full text-red-600 bg-red-50 text-xs font-medium border border-red-100'>Cancelled</span>
                                        : <span className='px-2.5 py-1 rounded-full text-green-600 bg-green-50 text-xs font-medium border border-green-100'>Confirmed</span>
                                    }
                                </div>
                            </div>
                        ))
                        ...
                        <div className="flex flex-col items-center justify-center py-20 h-full">
                            ...
                            {selectedDate ? (
                                <p className="text-gray-500 text-sm mt-2 max-w-xs text-center">
                                    No appointments scheduled for <span className="font-medium text-gray-900">{format(selectedDate, 'MMM do, yyyy')}</span>
                                </p>
                            ) : (

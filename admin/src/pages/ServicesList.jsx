...
const ServicesList = () => {
  ...
  return (
      <form onSubmit={onSubmitHandler} className="m-5 w-full">
        ...
        <div
            className="bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll scrollbar-hide">
          <div className="flex items-center gap-4 mb-8 text-gray-500">
            <label htmlFor="service-image">
              <img className="w-16 bg-transparent rounded-full cursor-pointer"
                   src={serviceImg ? URL.createObjectURL(serviceImg) : "/images/image-dropzone.jpg"} alt="upload-area"/>
            </label>
          ...
          <div className="flex flex-col gap-4 text-gray-600">
            <div className="flex flex-col gap-1">
              <p>Service Name</p>
              <input onChange={(e) => setType(e.target.value)} value={type} className="border rounded px-3 py-2"
                     type="text" placeholder="Service Name" required/>
            </div>

            <div className="flex flex-col gap-1">
              <p>Service Price</p>
              <input onChange={(e) => setFee(e.target.value)} value={fee} className="border rounded px-3 py-2"
                     type="number" placeholder="20" required/>
            </div>

            <div className="flex flex-col gap-1">
              <p>Short Description</p>
              <input onChange={(e) => setShortDescription(e.target.value)} value={shortDescription}
                     className="border rounded px-3 py-2" type="text" placeholder="Short summary" required/>
            </div>

            <div className="flex flex-col gap-1">
              <p>Description</p>
              <textarea onChange={(e) => setDescription(e.target.value)} value={description}
                        className="border rounded px-3 py-2" placeholder="Full service description" rows={4} required/>
            </div>

            <div className="flex flex-col gap-1">
              <p>Filter Category</p>
              <select onChange={(e) => setFilter(e.target.value)} value={filter} className="border rounded px-3 py-2">

...
const Services = () => {
  ...
  return (
    <div className='w-full p-5'>
      <p className='text-lg font-medium mb-4'>All Services</p>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 gap-y-6'>
        {currentServices.map((item, index) => (
          <div className='border rounded-xl overflow-hidden cursor-pointer group hover:translate-y-[-10px] transition-all duration-500' key={index}>
            <div className="relative">
              <img className='bg-blue-50 w-full h-48 object-cover' src={item.image} alt="" />
              <div className="absolute top-2 right-2 bg-white rounded-full p-1 shadow hover:bg-red-50" onClick={(e) => {
            ...
            <div className='p-4'>
              <div className="flex justify-between items-center mb-2">
                <span className={`text-xs px-2 py-1 rounded-full ${item.filter === 'дамско' ? 'bg-pink-100 text-pink-600' : item.filter === 'мъжко' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'}`}>{item.filter.toUpperCase()}</span>

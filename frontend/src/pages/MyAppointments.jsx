...
import { Calendar, Clock, Trash2, ChevronLeft, ChevronRight, PlusCircle } from "lucide-react";
import { useNavigate } from 'react-router-dom';
import axios from "axios";
import { toast } from "react-toastify";

const MyAppointments = () => {
  const { appointments, backendUrl, getAppointments, token } = useContext(AppContext)
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      if (window.confirm("Сигурни ли сте, че искате да отмените този час?")) {
        const { data } = await axios.post(backendUrl + '/api/appointment/cancel', { appointmentId }, { headers: { token } })
        if (data.success) {
        ...
        {currentAppointments.length > 0 ? (
          currentAppointments.map((item, index) => (
            ...
            >
              {/* Left: Small Image */}
              <div
                className="w-24 h-24 md:w-32 md:h-32 shrink-0 overflow-hidden rounded-2xl shadow-lg border border-white/20">
                <img
              ...
              <div className="flex-1 text-center md:text-left space-y-2">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-3">
                  <div
                    className="flex items-center gap-2 text-gray-400 text-sm bg-white/5 px-3 py-1 rounded-full border border-white/5">
              ...
              <div className="flex flex-col items-center md:items-end gap-4 w-full md:w-auto">
                ...
                {!item.cancelled ? (
                  ...
                  <button disabled className='px-6 py-2 rounded-xl text-red-500'>Отменен</button>
                )}
              </div>
            </div>
          ))
          ...
          <div
            className="text-center py-20 px-6 bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl shadow-xl">
            <div className="w-20 h-20 bg-amber-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
              <PlusCircle size={40} className="text-white" />
            </div>
            <h2 className="text-2xl font-bold text-gray-600 mb-2">Все още нямате записани часове</h2>
